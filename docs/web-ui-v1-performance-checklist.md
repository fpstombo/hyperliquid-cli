...
| _current branch_ | Signature motif + semantic icon pass (`dashboard`/`trade` shell) |            816.66 KB |                 808.25 KB |   not sampled in CI env | not sampled in CI env | not sampled in CI env | not sampled in CI env |               not sampled in CI env |           not sampled in CI env | not sampled in CI env | not sampled in CI env | no                        | n/a                          | `node scripts/ci/check-web-route-budgets.mjs`; visual audit via Playwright screenshots |
