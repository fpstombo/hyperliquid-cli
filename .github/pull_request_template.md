...
### Visual PR route-level enforcement (required for visual PRs in `apps/web`)

- [ ] I ran route-level checks for `/dashboard` and `/trade/[symbol]` against the thresholds in `docs/web-ui-v1-performance-checklist.md`.
- [ ] I verified polling-cadence update smoothness (prices + orders/positions) with no visible jitter in primary panels.
- [ ] If a visual choice regressed responsiveness, I simplified/removed that visual treatment in this same PR (no defer).
- [ ] I appended this PR's outcome row in the performance trend table before merge and linked it below.
