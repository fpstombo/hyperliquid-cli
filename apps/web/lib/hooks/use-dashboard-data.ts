...
export function useBalances(onTransientError?: (message: string) => void) {
  return usePollingResource({ fetcher: fetchBalances, pollMs: 5000, onTransientError })
}

export function usePositions(onTransientError?: (message: string) => void) {
  return usePollingResource({ fetcher: fetchPositions, pollMs: 5000, onTransientError })
}
